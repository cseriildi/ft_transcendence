...
describe('OAuth Routes', () => {
  ...
  describe('GET /oauth/github', () => {
    ...
    it('should set oauth_state cookie', async () => {
      ...
      expect(stateCookie?.path).toBe('/oauth')
    })

    it('should generate unique state for each request', async () => {
      const res1 = await app.inject({ method: 'GET', url: '/oauth/github' })
      const res2 = await app.inject({ method: 'GET', url: '/oauth/github' })
  ...
  describe('GET /oauth/github/callback', () => {
    it('should reject missing code parameter', async () => {
      const res = await app.inject({
        method: 'GET',
        url: '/oauth/github/callback?state=test-state',
      })
    ...
    it('should reject missing state parameter', async () => {
      const res = await app.inject({
        method: 'GET',
        url: '/oauth/github/callback?code=test-code',
      })
    ...
    it('should reject invalid state (CSRF protection)', async () => {
      const res = await app.inject({
        method: 'GET',
        url: '/oauth/github/callback?code=test-code&state=invalid-state',
        headers: {
    ...
    it('should create new user from GitHub OAuth', async () => {
      ...
      const initRes = await app.inject({ method: 'GET', url: '/oauth/github' })
    ...
    it('should login existing OAuth user', async () => {
      ...
      const initRes1 = await app.inject({ method: 'GET', url: '/oauth/github' })
      ...
      await app.inject({
        method: 'GET',
        url: `/oauth/github/callback?code=code1&state=${state1}`,
        headers: { cookie: `oauth_state=${stateCookie1?.value}` },
      })

      // Now login again with same OAuth user
      const initRes2 = await app.inject({ method: 'GET', url: '/oauth/github' })
      ...
      const res = await app.inject({
        method: 'GET',
        url: `/oauth/github/callback?code=code2&state=${state2}`,
    ...
    it('should handle GitHub token exchange failure', async () => {
      ...
      const res = await app.inject({
        method: 'GET',
        url: `/oauth/github/callback?code=bad-code&state=${state}`,
  ...
  describe('OAuth Integration with Other Endpoints', () => {
    it('should allow OAuth user to access protected endpoints', async () => {
      ...
      const profileRes = await app.inject({
        method: 'GET',
        url: `/users/${(authRes.json() as any).data.id}`,
        headers: {
