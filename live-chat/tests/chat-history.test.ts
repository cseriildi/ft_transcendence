...
describe("WebSocket - Chat History", () => {
  ...
  it("should store messages in chat history", async () => {
    ...
    ws1.send(JSON.stringify({ action: "send_message", chatid: "1-2", message: "Message 1" }));
    await waitForMessage(ws2);

    ws2.send(JSON.stringify({ action: "send_message", chatid: "1-2", message: "Message 2" }));
    await waitForMessage(ws1);
    ...
    const ws4 = new WebSocket(`${serverAddress}/ws?userId=3`);
    await new Promise((resolve) => ws4.on("open", resolve));
    ws4.send(JSON.stringify({ action: "join_chat", chatid: "1-2" }));
  ...
  it("should limit history to MAX_MESSAGES", async () => {
    ...
    for (let i = 0; i < 25; i++) {
      ws1.send(JSON.stringify({ action: "send_message", chatid: "1-2", message: `Message ${i}` }));
      await waitForMessage(ws2);
    }
    ...
    // Check history
    const ws3 = new WebSocket(`${serverAddress}/ws?userId=3`);
    await new Promise((resolve) => ws3.on("open", resolve));
    ws3.send(JSON.stringify({ action: "join_chat", chatid: "1-2" }));
